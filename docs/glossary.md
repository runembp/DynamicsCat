...
| Lookups Opener | A toggle tool that intercepts modifier+click events on populated lookup fields to open the referenced record in a background tab. The modifier key is configurable (default: Ctrl). | `lookups-opener`, `background.ts`, `state.ts` |
| Chrome commands | Chrome extension keyboard shortcuts registered in `manifest.json` under the `commands` key. Handled by `chrome.commands.onCommand` in the background worker. Users can customize them at `chrome://extensions/shortcuts`. | `manifest.json`, `background.ts` |
...
| Shortcut config | User-configurable modifier key combinations (e.g. `alt`, `ctrl+shift`) stored in `chrome.storage.local` and injected into page context via dataset attributes. Used by Override Readonly and Lookups Opener. | `background.ts`, `popup.ts`, `ribbon-toolbar`, `state.ts` |
