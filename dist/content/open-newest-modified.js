"use strict";(()=>{function S(e,d,l){requestAnimationFrame(()=>{let r=e.getBoundingClientRect();e.style.left=r.left+"px",e.style.top=r.top+"px",e.style.right="",e.style.transform=""});let o=!1,f=0,y=0,p=r=>{if(!o)return;let b=Math.max(0,Math.min(r.clientX-f,window.innerWidth-e.offsetWidth)),i=Math.max(0,Math.min(r.clientY-y,window.innerHeight-e.offsetHeight));e.style.left=b+"px",e.style.top=i+"px"},x=()=>{o=!1,d.style.cursor="move"};d.addEventListener("mousedown",r=>{l.contains(r.target)||(o=!0,f=r.clientX-e.offsetLeft,y=r.clientY-e.offsetTop,d.style.cursor="grabbing",r.preventDefault())}),document.addEventListener("mousemove",p),document.addEventListener("mouseup",x),new MutationObserver((r,b)=>{document.contains(e)||(document.removeEventListener("mousemove",p),document.removeEventListener("mouseup",x),b.disconnect())}).observe(document.body,{childList:!0,subtree:!0})}var P="crm-tools-toast-container";function g(e,d="info"){let l=document.getElementById(P);l||(l=document.createElement("div"),l.id=P,l.style.cssText=["position: fixed","bottom: 24px","right: 24px","z-index: 2147483647","display: flex","flex-direction: column","gap: 8px","pointer-events: none"].join("; "),document.body.appendChild(l));let o=document.createElement("div");o.style.cssText=["background: "+(d==="warn"?"#e65100":"#323232"),"color: #fff",'font-family: "Google Sans", Roboto, "Segoe UI", Arial, sans-serif',"font-size: 13px","padding: 10px 16px","border-radius: 6px","box-shadow: 0 2px 8px rgba(0,0,0,0.25)","pointer-events: auto","opacity: 1","transition: opacity 0.3s ease"].join("; "),o.textContent=e,l.appendChild(o),setTimeout(()=>{o.style.opacity="0",setTimeout(()=>o.remove(),350)},3500)}var t="crm-tools-newest-modified-panel",j="crm-tools-newest-modified-style",A="crm-tools-newest-modified-list",z=/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;function B(e){return parseInt(e.split(".")[0]??"8",10)>=9?"v9.0":"v8.2"}function v(e){return e.DisplayName?.UserLocalizedLabel?.Label??e.LogicalName}async function O(){let e=document.getElementById(t);if(e){e.remove();return}if(typeof Xrm>"u"||!Xrm.Page?.context)return;let d=Xrm.Page.context.getClientUrl(),l=B(Xrm.Page.context.getVersion());U();let o=document.createElement("div");o.id=t;let f=document.createElement("div");f.className="cnm-header";let y=document.createElement("span");y.className="cnm-title",y.textContent="\u{1F550} Jump to Latest";let p=document.createElement("button");p.className="cnm-close",p.textContent="\u2715",p.title="Close",p.addEventListener("click",()=>o.remove()),f.append(y,p);let x=document.createElement("div");x.className="cnm-body";let r=document.createElement("div");r.className="cnm-row";let b=document.createElement("label");b.className="cnm-label",b.textContent="Entity";let i=document.createElement("input");i.type="text",i.className="cnm-input",i.placeholder="Loading\u2026",i.disabled=!0,i.setAttribute("list",A),i.setAttribute("autocomplete","off");let E=document.createElement("datalist");E.id=A,i.addEventListener("keyup",n=>n.stopPropagation()),r.append(b,i,E);let w="modifiedon",h=document.createElement("div");h.className="cnm-row";let L=document.createElement("span");L.className="cnm-label",L.textContent="Sort by";let $=[],D=(n,c)=>{let a=document.createElement("button");return a.className="cnm-sort-btn"+(c===w?" cnm-sort-active":""),a.textContent=n,$.push(a),a.addEventListener("click",()=>{a.disabled||(w=c,$.forEach(s=>s.classList.remove("cnm-sort-active")),a.classList.add("cnm-sort-active"))}),a};h.append(L,D("Newest Modified","modifiedon"),D("Newest Created","createdon"));let C=document.createElement("div");C.className="cnm-row";let N=document.createElement("label");N.className="cnm-label",N.textContent="Record ID";let u=document.createElement("input");u.type="text",u.className="cnm-input",u.placeholder="Optional GUID\u2026",u.addEventListener("input",()=>{let n=z.test(u.value.trim());$.forEach(c=>{c.disabled=n})}),u.addEventListener("keyup",n=>n.stopPropagation()),u.addEventListener("keydown",n=>{n.key==="Enter"&&I(),n.stopPropagation()}),C.append(N,u);let T=document.createElement("div");T.className="cnm-row cnm-action-row";let m=document.createElement("button");m.className="cnm-open-btn",m.textContent="Open Record",m.disabled=!0,T.appendChild(m),x.append(r,C,h,T),o.append(f,x),document.body.appendChild(o),S(o,f,p);let k=[];try{k=(await(await fetch(`${d}/api/data/${l}/EntityDefinitions?$select=LogicalName,DisplayName,EntitySetName,PrimaryIdAttribute`)).json()).value.filter(a=>a.EntitySetName).sort((a,s)=>v(a).localeCompare(v(s)));for(let a of k){let s=document.createElement("option");s.value=v(a),s.label=a.LogicalName,E.appendChild(s)}i.placeholder="Type entity name\u2026",i.disabled=!1,m.disabled=!1}catch{i.placeholder="Failed to load entities",g("Could not load entity list.","warn");return}let I=async()=>{let n=i.value.trim().toLowerCase();if(!n){g("Enter an entity name.","warn");return}let c=k.find(s=>v(s).toLowerCase()===n||s.LogicalName.toLowerCase()===n);if(!c){g(`Entity "${i.value.trim()}" not found.`,"warn");return}let a=u.value.trim();if(z.test(a)){let s=a.replace(/^\{|\}$/g,"");window.open(`${d}/main.aspx?pagetype=entityrecord&etn=${c.LogicalName}&id=%7B${s}%7D`,"_blank"),o.remove();return}m.disabled=!0,m.textContent="Opening\u2026";try{let s=`${d}/api/data/${l}/${c.EntitySetName}?$select=${c.PrimaryIdAttribute}&$orderby=${w}%20desc&$top=1`;console.log("[DynamicsCat] OData query:",s);let M=await(await fetch(s,{headers:{Accept:"application/json","OData-MaxVersion":"4.0","OData-Version":"4.0"}})).json();if(!M.value?.length){g(`No records found for "${v(c)}".`,"warn");return}let R=(M.value[0][c.PrimaryIdAttribute]??"").replace(/^\{|\}$/g,"");if(!R){g("Could not determine record ID.","warn");return}window.open(`${d}/main.aspx?pagetype=entityrecord&etn=${c.LogicalName}&id=%7B${R}%7D`,"_blank"),o.remove()}catch{g("Failed to fetch record.","warn")}finally{m.disabled=!1,m.textContent="Open Record"}};m.addEventListener("click",()=>{I()}),i.addEventListener("keydown",n=>{n.key==="Enter"&&I(),n.stopPropagation()})}function U(){if(document.getElementById(j))return;let e=document.createElement("style");e.id=j,e.textContent=`
#${t} {
